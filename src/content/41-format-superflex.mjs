...
export default {
  ...
  claims: [
    ...
    'GREEN18 builds a Superflex board under two quarterback slots from the start rather than applying a quarterback multiplier to a single-quarterback board.',
    'Quarterback format, scoring rules and league size all shape the board before any player is valued, so the same Superflex setting produces a different board in a different league size or scoring rule.',
  ],
  ...
  blocks: [
    ...
    { type: 'prose', h2: 'How Does GREEN18 Handle the Superflex Setting?', body: [
      ...
      '**Concretely: Superflex is an assumption the board is built under, not a quarterback multiplier.** Setting Superflex does not scale quarterbacks up on a shared board; the board you draft from was built for two quarterback slots from the start. Your league size and scoring rules shape it alongside the quarterback format, which is why the same Superflex setting is a different board in a ten-team half-PPR league and a fourteen-team full-PPR one.',
  ...
  faq: [
    ...
    { q: 'Does GREEN18 apply a Superflex multiplier to quarterbacks?', a: 'No. Superflex is an assumption the board is built under rather than a multiplier applied to a single-quarterback board. Quarterback format, scoring rules and league size are all settled before any player is valued, so selecting Superflex gives you a board built for two quarterback slots rather than a shared one scaled up.' },
