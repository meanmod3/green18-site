...
export default {
  ...
  blocks: [
    ...
    { type: 'prose', h2: 'How Does GREEN18 Value Superflex Quarterbacks?', body: [
      ...
      'That phrase is literal. A Superflex board is built under two quarterback slots from the start, so no quarterback multiplier is ever applied to a shared board. The arithmetic above is not a correction layered on top of a single-quarterback ranking; it is the assumption the board was built under. How far [each format setting actually moves the board](/formats) is measured rather than assumed.',
  ...
  faq: [
    ...
    { q: 'Is the Superflex board a scaled version of the single-quarterback board?', a: 'No. A GREEN18 Superflex board is built under two quarterback slots rather than derived from a single-quarterback board by a positional multiplier, so the doubled starting demand is an assumption the board was built under rather than a correction applied afterward.' },
